...
'관리자 인증 필요' 문구가 있는 API에는 ZETIN 로그인 및 인증 과정을 거치고 나서 쿠키를 통한 JWT를 발급받은 후에야 접근할 수 있습니다. **HttpOnly 쿠키를 사용하기 때문에 클라이언트에서 스크립트로 접근할 수 없으며, 해당 API를 이용하여 JWT를 발급받고 쿠키를 요청할 수 있도록 합니다.**
